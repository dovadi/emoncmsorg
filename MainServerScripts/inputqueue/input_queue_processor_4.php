<?php
    ...
    */
    ...
    ini_set("error_log", "/var/log/emoncms/inputqueue_error4.log");

    define('EMONCMS_EXEC', 1);

    $fp = fopen("inputqueue4-lock", "w");
    ...
    while(true)
    {
        if ((time()-$ltime)>=1)
        {
            ...
            $buflength = $redis->llen('inputbuffer4');
            ...
            */
            ...
            $usleep = (int) $redis->get('usleep:4');
            if ($usleep<0) $usleep = 0;

            $redis->set('queue:wrktime4',$averagewrktime);
            $redis->set('queue:usleep4',$usleep);

            echo "Buffer length: ".$buflength." ".$usleep." ".$rn."\n";

            $redis->incrby('queue4:rn',$rn);
            $rn = 0;

            if ($redis->get('stopinputqueue4')==1) {
              $redis->set('stopinputqueue4',0);
              die;
            }
        }
        ...
        if ($redis->llen('inputbuffer4')>0)
        {
            // check if there is an item in the queue to process
            $line_str = $redis->lpop('inputbuffer4');
        }
        ...
        if ($line_str)
        {
            ...
            // Shift users out of queue 4 into queue 3
            if ($userid<15900) {
