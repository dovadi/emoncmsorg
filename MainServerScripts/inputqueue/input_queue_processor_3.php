<?php
    ...
    */
    ...
    ini_set("error_log", "/var/log/emoncms/inputqueue_error3.log");

    define('EMONCMS_EXEC', 1);

    $fp = fopen("inputqueue3-lock", "w");
    ...
    while(true)
    {
        if ((time()-$ltime)>=1)
        {
            ...
            $buflength = $redis->llen('inputbuffer3');
            ...
            */
            ...
            $usleep = (int) $redis->get('usleep:3');
            if ($usleep<0) $usleep = 0;

            $redis->set('queue:wrktime3',$averagewrktime);
            $redis->set('queue:usleep3',$usleep);

            echo "Buffer length: ".$buflength." ".$usleep." ".$rn."\n";

            $redis->incrby('queue3:rn',$rn);
            $rn = 0;

            if ($redis->get('stopinputqueue3')==1) {
              $redis->set('stopinputqueue3',0);
              die;
            }
        }
        ...
        if ($redis->llen('inputbuffer3')>0)
        {
            // check if there is an item in the queue to process
            $line_str = $redis->lpop('inputbuffer3');
        }
