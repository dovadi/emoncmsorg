...
var table = {
  ...
  'draw':function() {
    ...
    for (group in groups) {
      ...
      for (field in table.fields)
      {
        ...
        // Persistant column hide 
        var display = ""; if (table.fields[field].display!=undefined && table.fields[field].display==false) display = "style='display:none'";

        html += "<th "+display+" ><a type='sort' field='"+field+"'>"+title+"</a></th>";
      }
      html += "</tr>";
      html += groups[group];
      html += "</tbody>";
  ...
  'draw_row': function(row) {
    ...
    for (field in table.fields) {
        ...
        var display = ""; if (table.fields[field].display!=undefined && table.fields[field].display==false) display = "style='display:none'";
        // Draw the row
        html += "<td row='"+row+"' field='"+field+"' "+display+" >"+table.fieldtypes[table.fields[field].type].draw(row,field)+"</td>";
    }
    html += "</tr>";
    return html;
  },
