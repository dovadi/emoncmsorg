...
/*
 ...
 Emoncms.org change log:
 - commented EmonLogger
 - commented set_timezone_offset
 
 - changed: 
    $list[10] = array(_("update feed @time"),ProcessArg::FEEDID,"update_feed_data",1,DataType::DAILY,"Input",array(Engine::MYSQL));
   to:
    $list[10] = array(_("update feed @time"),ProcessArg::FEEDID,"update_feed_data",1,DataType::UNDEFINED,"Deleted");
    
 - commented log info
 - added: if (count($pairs)>50) return false;
 - commented update_feed_data($id, $time, $value)
...
class Process
{
    ...
    public function get_process_list()
    {
        ...
        $list[1] = array(_("Log to feed"),ProcessArg::FEEDID,"log_to_feed",1,DataType::REALTIME,"Main",array(Engine::PHPFINA,Engine::PHPFIWA,Engine::PHPTIMESERIES));                  
        ...
        $list[4] = array(_("Power to kWh"),ProcessArg::FEEDID,"power_to_kwh",1,DataType::REALTIME,"Main",array(Engine::PHPFINA));               
        $list[5] = array(_("Power to kWh/d"),ProcessArg::FEEDID,"power_to_kwhd",1,DataType::DAILY,"Daily (UTC only)",array(Engine::PHPTIMESERIES));               
        $list[6] = array(_("x input"),ProcessArg::INPUTID,"times_input",0,DataType::UNDEFINED,"Input");                   
        $list[7] = array(_("Input on-time"),ProcessArg::FEEDID,"input_ontime",1,DataType::DAILY,"Deleted",array(Engine::PHPTIMESERIES));                 
        $list[8] = array(_("Wh increments to kWh/d"),ProcessArg::FEEDID,"kwhinc_to_kwhd",1,DataType::DAILY,"Deleted",array(Engine::PHPTIMESERIES));      
        ...
        $list[15] = array(_("Rate of change"),ProcessArg::FEEDID,"ratechange",1,DataType::REALTIME,"Deleted",array(Engine::PHPFIWA,Engine::PHPFINA,Engine::PHPTIMESERIES));               
        $list[16] = array(_("Histogram"),ProcessArg::FEEDID,"histogram",2,DataType::HISTOGRAM,"Deleted",array(Engine::MYSQL));                   
        ...
        $list[20] = array(_("Total pulse count to pulse increment"),ProcessArg::FEEDID,"pulse_diff",1,DataType::REALTIME,"Deleted",array(Engine::PHPFINA,Engine::PHPTIMESERIES));
        
        // fixed works now with redis - look into state implementation without feed
        $list[21] = array(_("kWh to Power"),ProcessArg::FEEDID,"kwh_to_power",1,DataType::REALTIME,"Deleted",array(Engine::PHPFIWA,Engine::PHPFINA,Engine::PHPTIMESERIES));
        
        $list[22] = array(_("- input"),ProcessArg::INPUTID,"subtract_input",0,DataType::UNDEFINED,"Input");               
        $list[23] = array(_("kWh to kWh/d"),ProcessArg::FEEDID,"kwh_to_kwhd",2,DataType::DAILY,"Deleted",array(Engine::PHPTIMESERIES));                  // fixed works now with redis
        ...
        $list[27] = array(_("Max value"),ProcessArg::FEEDID,"max_value",1,DataType::DAILY,"Daily (UTC only)",array(Engine::PHPTIMESERIES));                        
        $list[28] = array(_("Min value"),ProcessArg::FEEDID,"min_value",1,DataType::DAILY,"Daily (UTC only)",array(Engine::PHPTIMESERIES));  
        ...
        $list[34] = array(_("Wh Accumulator"),ProcessArg::FEEDID,"wh_accumulator",1,DataType::REALTIME,"Main",array(Engine::PHPFINA));
    ...
    public function input($time, $value, $processList)
    {
        ...
        if (count($pairs)>50) return false;
        ...
        foreach ($pairs as $pair)
        {
            ...
            if (!isset($process_list[$processid])) return false;
	...
	*/
    ...
    public function power_to_kwh($feedid, $time_now, $value)
    {
        ...
        if ($last===false) $last = array('time'=>$time_now,'value'=>0);
    ...
    public function kwh_to_kwhd($feedid, $time_now, $value)
    {
        ...
        if ($currentkwhd===false) $currentkwhd = array('time'=>$time_now,'value'=>$value);
    ...
    public function input_ontime($feedid, $time_now, $value)
    {
        // Get last value
        $last = $this->feed->get_timevalue($feedid);
        if ($last===false) $last = array('time'=>$time_now,'value'=>$value);
    ...
    {
        $last = $this->feed->get_timevalue($feedid);
        if ($last===false) $last = array('time'=>$time,'value'=>0);
    ...
    public function histogram($feedid, $time_now, $value)
    {
        ...
        if ($lastvalue===false) $lastvalue = array('time'=>$time_now,'value'=>0);
    ...
    {
        $last = $this->feed->get_timevalue($feedid);
        if ($last===false) $last = array('time'=>0, 'value'=>0);
    ...
    {
        $last = $this->feed->get_timevalue($feedid);
        if ($last===false) $last = array('time'=>0, 'value'=>1);
   ...
   public function divide_by_feed($feedid, $time, $value)
    ...
    public function wh_accumulator($feedid, $time, $value)
    {
        ...
        if ($redis->exists("process:whaccumulator:$feedid")) {
            ...
            if ($last_feed ===false) $last_feed = array('time'=>$time, 'value'=>$value);
            ...
            $power = 0;
            if ($time_diff>0) $power = ($val_diff * 3600) / $time_diff;
